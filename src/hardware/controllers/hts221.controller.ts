...
import { GPHTS221Data } from '../models';
...
export class GPHTS221Controller extends EventEmitter {
  ...
  private async _start(i2cBus: PromisifiedBus) {
    try {
      if (await this._isHTS221(i2cBus)) {
        ...
        this._logger.info(`[Hardware / Sensors/HTS221 Controller] Humidity and temperature sensors found at address ${toHex(HTS221_ADDRESS)}`);
        this._logger.debug(`[Hardware / Sensors/HTS221 Controller] Tempetarure calibration (t_read * M + C) M: ${this._temperatureM} C: ${this._temperatureC}`);
        this._logger.debug(`[Hardware / Sensors/HTS221 Controller] Humidity calibration (h_read * M + C) M: ${this._humidityM} C: ${this._humidityC}`);
        this.emit('ready');
      } else {
        this._logger.error(`[Hardware / Sensors/HTS221 Controller] Sensor not found at address ${toHex(HTS221_ADDRESS)}`);
        this.emit('notFound');
      }
    } catch (error) {
      this._logger.error(`[Hardware / Sensors/HTS221 Controller] Initialization failed ${error}`);
  ...
  private async _readHumidity(i2cBus: PromisifiedBus): Promise<number> {
    ...
    this._logger.debug(`[Hardware / Sensors/HTS221 Controller] Humidity read: ${humidity}% (H/L: ${toBinary(humidityOutH)}/${toBinary(humidityOutL)})`);
    return humidity;
  }
  ...
  private async _readTemperature(i2cBus: PromisifiedBus): Promise<number> {
    ...
    this._logger.debug(`[Hardware / Sensors/HTS221 Controller] Temperature (from humidity) read: ${temperature}°c (H/L: ${toBinary(temperatureOutH)}/${toBinary(temperatureOutL)})`);
    return temperature;
  }
