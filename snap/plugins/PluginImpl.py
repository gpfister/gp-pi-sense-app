...
class PluginImpl(PluginV2):
    ...
    def _get_node_command(self) -> str:
        ...
        version = self.options.node_version
    ...
    def get_build_commands(self) -> List[str]:
        return [
            ...
            'rm ${SNAPCRAFT_PART_INSTALL}/*.md',
            'mv *.md ${SNAPCRAFT_PART_INSTALL}',
            'npm install -g npm node-gyp --prefix=${SNAPCRAFT_PART_INSTALL}',
            'rm -rf dist node_modules'
            'npm ci',
            'npm run lint',
            'npm run build',
            'mkdir build',
            'npm install -g $(npm pack . | tail -1) --production --prefix=./build',
            'mv ./build/lib/node_modules/* ${SNAPCRAFT_PART_INSTALL}/lib/node_modules/',
            'mv ./build/bin/* ${SNAPCRAFT_PART_INSTALL}/bin'
        ]
