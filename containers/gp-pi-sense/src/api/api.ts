...
import { GPSensorsRouter, GPHomeKitRouter } from "./routes";
...
export class GPAPI {
  ...
  private async _checkRequirements() {
    ...
    if (!process.env.GP_API_DATA_FOLDER_PATH) {
      this._logger.error(`Missing required GP_DATA_FOLDER_PATH environment variable`);
      success = false;
    } else {
      // Check the folder
      if (!fs.existsSync(process.env.GP_API_DATA_FOLDER_PATH)) {
        if (fs.mkdirSync(process.env.GP_API_DATA_FOLDER_PATH, { recursive: true })) {
          this._logger.info(`Created data folder: ${process.env.GP_API_DATA_FOLDER_PATH}`);
        } else {
          this._logger.error(`Data folder path not set (env: GP_API_DATA_FOLDER_PATH = "${process.env.GP_API_DATA_FOLDER_PATH}")`);
          success = false;
        }
      } else {
        this._logger.info(`Found data folder: ${process.env.GP_API_DATA_FOLDER_PATH}`);
      }
  ...
  async start() {
    ...
    this._express.use("/api/homekit", GPHomeKitRouter.getRouter());
    this._express.use("/api/sensors", GPSensorsRouter.getRouter());
