...
import { GPHomekitCredentials, GPSensorData } from '../models';
...
export class GPLocalApiHttpController {
  ...
  private async _get(query: string): Promise<axios.AxiosResponse> {
    this._logger.info(`[Homekit / Local HTTP Controller] GET Query ${query}`);
    const response = await axios.default.get(query);
    this._logger.debug(`[Homekit / Local HTTP Controller] GET Query ${query}: ${response.status} -> ${JSON.stringify(response.data)}`);
    return response;
  }
  ...
  //   this._logger.info(`[Local API HTTP Controller] POST Query ${query}: ${JSON.stringify(payload)}`);
  //   const response = await axios.default.post(query, payload);
  //   this._logger.debug(`[Local API HTTP Controller] POST Query ${query}: ${response.status} -> ${JSON.stringify(response.data)}`);
  //   return response;
  // }
  ...
  //   this._logger.info(`[Homekit / Local HTTP Controller] PATCH Query ${query}: ${JSON.stringify(payload)}`);
  //   const response = await axios.default.patch(query, payload);
  //   this._logger.debug(`[Homekit / Local HTTP Controller] PATCH Query ${query}: ${response.status} -> ${JSON.stringify(response.data)}`);
  //   return response;
  // }
  ...
  //   const query = `${this._baseUrl}/api/sensor`;
  //   await this._post(query, sensorData);
  // }

  async getLatestSensorData(): Promise<GPSensorData> {
    const query = `${this._baseUrl}/api/sensors`;
    const result = await this._get(query);
    return result.data;
  }

  async getHomekitCredentials(): Promise<GPHomekitCredentials> {
    const query = `${this._baseUrl}/api/homekit/credentials`;
    const result = await this._get(query);
    return result.data;
  }
}
