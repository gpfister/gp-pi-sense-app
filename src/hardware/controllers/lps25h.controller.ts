...
import { GPLPS25HData } from '../models';
...
export class GPLPS25HController extends EventEmitter {
  ...
  constructor(logger: Logger) {
    ...
    openPromisified(1)
      ...
      .catch((error) => {
        this._logger.error(`[Hardware / Sensors/LPS25H Controller] Initialization failed ${error}`);
  ...
  private async _start(i2cBus: PromisifiedBus) {
    try {
      if (await this._isLPS25H(i2cBus)) {
        ...
        this._logger.info(`[Hardware / Sensors/LPS25H Controller] Pressure sensor found at address ${toHex(LPS25H_ADDRESS)}`);
        this.emit('ready');
      } else {
        this._logger.error(`[Hardware / Sensors/LPS25H Controller] Sensor not found at address ${toHex(LPS25H_ADDRESS)}`);
        this.emit('notFound');
      }
    } catch (error) {
      this._logger.error(`[Hardware / Sensors/LPS25H Controller] Initialization failed ${error}`);
  ...
  private async _readPressure(i2cBus: PromisifiedBus): Promise<number> {
    ...
    this._logger.debug(`[Hardware / Sensors/LPS25H Controller] Pressure read: ${pressure}hpa (H/L/XL: ${toBinary(pressureOutH)}/${toBinary(pressureOutL)}/${toBinary(pressureOutXL)})`);
    return pressure;
  }
  ...
  private async _readTemperature(i2cBus: PromisifiedBus): Promise<number> {
    ...
    this._logger.debug(`[Hardware / Sensors/LPS25H Controller] Temperature (from pressure) read: ${temperature}°c (H/L: ${toBinary(temperatureOutH)}/${toBinary(temperatureOutL)})`);
    return temperature;
  }
