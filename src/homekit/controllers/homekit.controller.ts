...
const VERSION = require('../../../package.json') as string;
  ...
  private async startAccessory() {
    ...
    currentTemperatureCharacteristic.on(CharacteristicEventTypes.GET, async (callback: CharacteristicGetCallback) => {
      try {
        ...
        this._logger.error(`[Homekit / Homekit Controller] Unable to get latest sensor data: ${error}`);
    ...
    currentRelativeHumidityCharacteristic.on(CharacteristicEventTypes.GET, async (callback: CharacteristicGetCallback) => {
      try {
        const sensorData = await this._localApiHttpController.getLatestSensorData();
        callback(undefined, sensorData.humidity >= 0 && sensorData.humidity <= 100 ? sensorData.humidity : 0);
      } catch (error) {
        this._logger.error(`[Homekit / Homekit Controller] Unable to get latest sensor data: ${error}`);
        callback(undefined, 0);
      }
    ...
    this._logger.info(`[Homekit / Homekit Controller] Username: ${credentials.username}`);
    this._logger.info(`[Homekit / Homekit Controller] Pincode: ${credentials.pincode}`);
    ...
    this._logger.info(`[Homekit / Homekit Controller] Accessory '${this._accessory.displayName}' (uuid: ${this._accessory.UUID}) is ready`);
  }
}
